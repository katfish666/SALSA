...
def draw_loss_plot(tag, use_losses, bs, e, y_lim=(0,5), figsize=(12.5,5), save=True):
    ...
    losses = pd.read_csv(f'{p}results/training_logs/losses_{tag}.csv',usecols=use_losses)
    ...
    if save:
        plt.savefig(f'{p}results/training_logs/loss_plot_{tag}.png', bbox_inches='tight')
    plt.show()
...
def eval_loss_plot(tag, which_train, use_losses, bs, n_epochs, 
    ...
    anc_path = f'{p}data/model_ready/{which_train}/train/anchor_smiles.csv'
    aug_path = f'{p}data/model_ready/{which_train}/train/augmented_smiles.csv'
    ...
    path = f'{p}results/models/{tag}/{n_epochs-1:02}.pt'
