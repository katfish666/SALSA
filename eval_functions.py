...
def get_df_and_latents(tag, which_train, which_test, test_ood, test_extended, samp_size, eval_bs, 
    ...
    if test_ood and test_extended:
        raise ValueError("Must choose OOD or extended.")
            
    elif test_extended:
        anc_path = f'{mdir}{which_test}/test/anchor_smiles_extended.csv'
        ds = ContraSeqDataset(anc_path)
        _df = get_dataset_array(anc_path)
        ds_len = len(_df)
    ...
    # # # Take smaller sample of eval dataset if requested.
    if samp_size < ds_len:
        rand = random.sample(range(0,ds_len),samp_size)
        if not test_extended:
...
def get_umap_tag(tag, which_test, test_ood, test_extended, samp_size):
    special = 'ood' if test_ood else ''
    special = 'extended' if test_extended else special
    which = 'train' if which_test=='train' else 'test'
    which = special if special else which
