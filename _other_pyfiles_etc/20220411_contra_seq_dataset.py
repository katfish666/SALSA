...
class SeqDatum(object):
    def __init__(self, smi, atype, idx, smi_tokens,
             start_token='<', 
             end_token='>', 
             pad_token = 'X',
             max_len = 120,
...
#         for k,v in self.seq_attr.items():
#             self.seq_attr[k] = torch.Tensor(v)
    ...
    def __getitem__(self,idx):
        if isinstance(idx, slice):
            ...
            for idx in iter(idc):
                ...
                anc_seq = SeqDatum(anc_smi,'Anc',idx,self.tokens).__getitem__()

                aug_smis = self.aug_smi[self.aug_smi[:,0]==idx][:,1]
                aug_seqs = [SeqDatum(sm,'Aug',idx,self.tokens).__getitem__() for sm in aug_smis]
